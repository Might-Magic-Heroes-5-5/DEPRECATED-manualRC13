class Tooltip < Shoes::Widget
		...
		def show ( options={} )
			offset, high, wide, text, text2, header, back1, back2 = 10, options[:height], options[:width], options[:text] || "ERROR: Missing mandatory options \"width:\", \"height:\" or \"text:\"\nAvailable options:\ntext: the text presented in the popup window\nwidth: the width of the popup\nheight: the height of the popup\nfont: the font of the text\nstroke: the stroke of the text\nsize: the size of the text\nborder: the colour of the border of the tooltip\nbackground: the colour of the tooltip box\nExample:   slot.hover { @tooltip.show text: \"Popup comming through\", width: 200, height: 100, border: rgb(0,20,5,0.5), background: rgb(0,244,99,0.5) , font: \"Vivaldi\" , size: 10, stroke: white }", options[:text2] || "", options[:header] || "", options[:border] || rgb(120,42,5,0.5), options[:background] || rgb(180,150,110,0.7) 
...
class MyApp < Shoes
			...
			box[y].clear do
				unless perks[y] == "" || perk_value == "2"
					...
					contents[1].hover do
						...
						@@sign.show text: ( File.read("text/skills/#{perks[y]}/Description.txt") ), header: ( File.read("text/skills/#{perks[y]}/name.txt") ), text2: text2
					end	
					contents[1].leave do
	...
	def set	(img, options={} )
		img.hover { @@sign.show  text: options[:text], header: options[:header] , text2: options[:text2], width: options[:width], height: options[:height] }	
		img.leave { @@sign.hide }
	end	
...
########################## URLS START HERE #####################################################################		
	def herobuilder
		...
		stack width: 280 do		#################################################### LEFT  - LISTBOX OF HEROES, PRIMARY STATS
			image "pics/themes/manuscript.png", left: 8, top: 244, width: 263
			image "pics/themes/factions_pane.png", left: 19, top: 39, width: 295, height: 170
			for i in 0..7 do
				top = i/4*51;	left = i*51 - 4*top
				flow left: 65 + left, top: 75 + top, width: 50, height: 50 do
					set_factions ( image "pics/factions/#{factions[i]}.png", width: 50 ), factions[i]
				end
			end
			subtitle "Faction Classes", left: 30, top: 250, font: "Vivaldi"
			@class_board = flow left: 30, top: 300, width: 210, height: 45;				 ############# CLASSES WINDOW
			@primary_board = flow width: 220, left: 45, top: 365 do					 ############# PRIMARY STATS TABLE
			...
			flow do
				@stat_damage = para "", left: 52, top: 505, size: 11		
				@stat_defense = para "", left: 129, top: 505, size: 11	
				@stat_mana = para "", left: 205, top: 505, size: 11
				subtitle  "Hero level", left: 95, top: 540, size: 16
				@box_level = flow width: 40, height: 35, left: 196, top: 40
				set ( image "pics/stats/damage.png", left: 37, top: 10, width: 15 ), text: "3.33%*Attack*(1+offense_skill) damage boost"
				set ( image "pics/stats/defense.png", left: 114, top: 10, width: 15 ), text: "3.33%*Defense*(1+defense_skill) health boost"
				set ( image "pics/stats/mana.png", left: 190, top: 10, width: 15 ), text: "Hero Mana pool"
				set @box_level, text: "Left click to level up!", width: 200, height: 30
			end
		end
...
Shoes.app(title: "Open Skillwheel v:0.75, database: HoMM 5.5 RC8b", width: 1000, height: 730, resizable: false )
